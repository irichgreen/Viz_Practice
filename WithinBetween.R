...
n1<-10000  #set the sample size in group 1
n2<-10000  #set the sample size in group 2
...
#This formula creates a second sample with specific mean and sd, which is correlated as desired with the first sample.
y <- (cor.true * (x-(mean(x))) + sqrt(1 - cor.true^2) * rnorm(n = n2, mean = my, sd = sdy)) + my*(1-sqrt(1 - cor.true^2))
dif<-x-y
